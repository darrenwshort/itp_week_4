...
##################################### FNs ##########################################
def write_poke_cells(abilities_dict, sheet, start_col = 1, start_row = 1):
    
    c, r = start_col, start_row  # initialize col(c) and row(r)
    curr_cell = sheet.cell(row=r, column=c)
    curr_cell.font = Font(size="14", bold=True, italic=True, underline="single")
    curr_cell.value = "Character Name"

    for char_name in sorted(abilities_dict.keys()):
        r+=1
        curr_cell = sheet.cell(row=r, column=1)
        curr_cell.value = char_name
        abil_list = poke_abilities[char_name]     # get list of abilities
        abil_len = len(poke_abilities[char_name]) # get length of abilities list
        abil_count = 1 # used to represent current ability num
        c = 2 # reset column to 2 for begin of ability header write
        for ability in abil_list:
            r = 1 # reset row to 1 for begin of ability header write
            curr_cell = sheet.cell(row=r, column=c)
            curr_cell.font = Font(size="14", bold=True, italic=True, underline="single")
            curr_cell.value = f"Ability {abil_count}" # write header for current ability
            r+=1 # increment 'r' to move to next row for ability write
            curr_cell = sheet.cell(row=r, column=c)
            curr_cell.value = ability
            c+=1 # increment 'c' to move to next column for ability write
            abil_count+=1 # increment 'count' for next ability
        c = 1 # reset column to 1, for next write of Character Name
...
start_col = 2
start_row = 2
write_poke_cells(poke_abilities, sheet, start_col, start_row)
#
# 
